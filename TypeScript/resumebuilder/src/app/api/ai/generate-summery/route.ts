...
// Strips markdown code fences and parses JSON-wrapped AI responses into plain text
function extractPlainText(raw: string): string {
  ...
  // If it looks like JSON, try to parse it and pull out the text value
  if (text.startsWith("{") || text.startsWith("[")) {
    try {
      ...
      if (parsed && typeof parsed === "object") {
        ...
        const value =
          parsed.summery ?? parsed.summary ?? parsed.text ?? parsed.content;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const summery = extractPlainText(result);
